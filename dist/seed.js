...
async function seedDatabase() {
    try {
        ...
        for (const planData of plans) {
            const existingPlan = await models_1.Plan.findOne({ name: planData.name });
            if (!existingPlan) {
                const newPlan = new models_1.Plan(planData);
                await newPlan.save();
        ...
        for (const userData of usersToCreate) {
            const userDoc = new models_1.User(userData);
            await userDoc.save();
        ...
        const createdProjects = await models_1.Project.insertMany(projectsToCreate);
        ...
        const createdCourses = await models_1.Course.insertMany(coursesToCreate);
